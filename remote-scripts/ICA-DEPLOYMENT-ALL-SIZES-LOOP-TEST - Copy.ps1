        ...
        While ($count -lt $DeploymentCount)
        {
            ...
            if ($deployedServiceName -or $deployedResourceGroupName)
            {
                if ( $UseAzureResourceManager )
                {
                        ...
                        $expectedVMSize = Get-AzureRmVMSize -Location $allVMData.Location | Where-Object {$_.Name -eq $allVMData.InstanceSize}
                        ...
                        { 
                            ...
                            if(($expectedRAMSizeinMB - $actualRAMSizeinMB) -le 500)
                            {
                                LogMsg "RAM Size verification is SUCCESS"
